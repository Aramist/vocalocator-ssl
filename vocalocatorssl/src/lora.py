...
class LORA_Conv1d(nn.Module):
    def __init__(self, mod: nn.Conv1d, rank: int, alpha: float = 1.0):
...
class LORA_Linear(nn.Module):
    def __init__(self, mod: nn.Module, rank: int, alpha: float = 1.0):
        super(LORA_Linear, self).__init__()
        self.wrapped = mod
        self.rank = rank
        scale = torch.tensor(alpha / rank, dtype=torch.float32)
        # Register scale as a buffer so it is moved to the right device
    ...
    def _make_matrix(self) -> Tensor:
        ...
        W_comb = torch.addmm(
            self.w_wrap, self.w_lora_B, self.w_lora_A, alpha=self.scale
        )
        return W_comb
...
class LORA_MHA(nn.Module):
    ...
    def __init__(self, mod: nn.MultiheadAttention, rank: int, alpha: float = 1.0):
